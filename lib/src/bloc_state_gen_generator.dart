...
class BlocStateGenGenerator extends GeneratorForAnnotation<BlocStateGen> {
  @override
  ...
  void _validateElement(Element element) {
    if (element is! ClassElement) {
      throw InvalidGenerationSourceError(
        ...
        element: element,
        todo: 'Apply @BlocStateGen annotation to a Bloc or Cubit class',
      );
    }
    ...
    if (supertype == null ||
        (!supertype.element.name.contains('Bloc') &&
            !supertype.element.name.contains('Cubit'))) {
      throw InvalidGenerationSourceError(
        'BlocStateGen must be applied to a class that extends Bloc or Cubit.',
        element: element,
  ...
  void _writeFileHeader(StringBuffer buffer, String fileName) {
    ...
    buffer.writeln(
        '// ignore_for_file: unused_element, deprecated_member_use, unused_local_variable, non_constant_identifier_names');
    buffer.writeln();
  ...
  List<ClassElement> _getSubclasses(ClassElement element) {
    final allClasses =
        element.library.topLevelElements.whereType<ClassElement>();
    final parentBloc = element.enclosingElement3;
  ...
  String _getParameterName(String className) {
    ...
    final cleanName = className
        .replaceAll(RegExp(r'State$'), '')
        .replaceAll(RegExp(r'Bloc$'), '')
        .replaceAll(RegExp(r'Cubit$'), '');
